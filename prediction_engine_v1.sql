...
alter table public.participant_predictions
  ...
  add column if not exists predicted_home_penalties integer check (predicted_home_penalties between 0 and 20),
  add column if not exists predicted_away_penalties integer check (predicted_away_penalties between 0 and 20);
alter table public.prediction_fixtures
  add column if not exists result_winner_team_id uuid references public.teams(id),
  add column if not exists result_home_penalties integer check (result_home_penalties between 0 and 20),
  add column if not exists result_away_penalties integer check (result_away_penalties between 0 and 20),
