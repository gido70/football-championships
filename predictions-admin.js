...
async function loadPredictionStatus(){
  ...
  const cards=rounds.filter(r=>!r.is_test).map(r=>{const rows=fixtures.filter(f=>f.round_id===r.id);return `<div class="panel result-card"><h3>${adminEsc(r.name)}</h3><p>${rows.length} مباراة · ${rows.filter(f=>f.is_scored).length} نتيجة محتسبة</p><div class="status good">${r.status==='open'?'✓ الجولة مفتوحة للترشيحات':'حالة الجولة: '+adminEsc(r.status)}</div></div>`;}).join('');
  const lockText=(setting?.lock_minutes??0)===0?'فور بدء المباراة':'قبل المباراة بـ'+setting.lock_minutes+' دقائق';
  document.getElementById('fixtures').innerHTML=`<div class="panel result-card"><h3>${setting?.enabled?'✅ المزامنة مفعلة':'⏸ المزامنة متوقفة'}</h3><p><strong>بداية الترشيحات:</strong> ${start}</p><p><strong>قفل الترشيح:</strong> ${lockText}</p></div>${cards||(ADMIN_TOURNAMENT_ID==='eee33333-5d2a-4f3b-a981-d4b8f5f86143'?'<div class="panel result-card"><h3>🔒 الترشيحات لم تبدأ</h3><p>ستُفتح تلقائيًا عند وصول دوري أبطال أوروبا إلى دور الـ16.</p></div>':'<div class="panel result-card"><h3>⏳ بانتظار الجدول الحقيقي</h3><p>عند تسجيل مباراة بتاريخ ووقت وفريقين ستظهر هنا وفي صفحة المشاركين تلقائيًا.</p></div>')}`;
