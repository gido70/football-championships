...
const publicPage=read('playstation.html'),adminPage=read('playstation-admin.html'),operatorPage=read('playstation-operator.html'),operatorManifest=read('manifest-playstation-operator.webmanifest'),serviceWorker=read('sw.js'),tournament=read('tournament.html'),home=read('admin-home.html'),sql=read('supabase/migrations/20260915211500_playstation_competition.sql'),multiDeviceSql=read('supabase/migrations/20260916073000_playstation_operator_multidevice_schedule.sql'),singleTrialSql=read('supabase/migrations/20260916074500_disable_duplicate_playstation_trial_operator.sql');
...
must(multiDeviceSql.includes('allow_multiple_devices boolean not null default false')&&multiDeviceSql.includes('a.allow_multiple_devices'),'استثناء حساب التجربة متعدد الأجهزة غير مؤمّن');
must(singleTrialSql.includes("set is_active=false")&&singleTrialSql.includes('المشغّل التجريبي — الكمبيوتر'),'حساب التجربة الاحتياطي لم يُعطّل');
